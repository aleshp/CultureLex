...
// Иконки для мобильного (и будущего десктопного) меню
const navItems =[
  { 
    page: 'Home', 
    path: '/', 
  ...
  const renderNavItems = role === 'teacher' ?[
    { page: 'Dashboard', path: '/teacher', icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg> },
  { 
    page: 'Library', 
    path: '/categories', 
...
export function Navbar() {
  ...
  // Достаем нужные данные из нашего хранилища
  const { stats, session, role, classGroup, setSession } = useStore()
      ...
      <header className="fixed top-0 left-0 right-0 z-50 glass-panel">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2">
            ...
            {/* На мобилках скрываем текст логотипа, чтобы влезли кнопки */}
          ...
          <div className="flex items-center gap-3 sm:gap-4 md:border-l md:border-white/10 md:pl-6">
            ...
            {/* Иконка Огня (Дни подряд) - Видна всегда */}
            ...
            {/* Авторизация: Кнопка Входа / Выхода */}
